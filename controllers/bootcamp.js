...
exports.createBootcamp = asyncHandler( async (req, res, next) => {
  ...
  const plublishedBootcamp = await Bootcamp.findOne({user: req.user.id})

 // If the user is not an admin, they can only add one bootcamp
  if( plublishedBootcamp && req.user.role !== 'admin')  {
...
exports.bootcampPhotoUpload = asyncHandler (async (req, res, next) => {
  ...
  const bootcamp = await Bootcamp.findById(req.params.id)   
    ...
    if(!bootcamp) {
        ...
        //make sure the user is the bootcamp owner
   if( bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin' ) {
    return  next(new ErrorResponse(`User ${req.params.id} is not authorized to upload photo for this bootcamp`, 401))
   }
